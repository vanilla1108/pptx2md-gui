    ...
    try:
        ...
        if len(err.args) > 0 and re.match(r'There is no item named .*NULL.* in the archive', str(err.args[0])):
            ...
            try:
                ...
                logger.error(
                    'failed to purge corrupted links, you can report this at https://github.com/vanilla1108/pptx2md-gui/issues')
                raise err
        else:
            logger.error('unknown error, you can report this at https://github.com/vanilla1108/pptx2md-gui/issues')
            raise err
    return prs
