...
def setup_logging(compat_tqdm=True, external_handlers=None, level=logging.INFO):
    """设置日志配置。

    参数:
        compat_tqdm: 使用 TqdmStreamHandler 以兼容 CLI 中的 tqdm 进度条。
        external_handlers: 要添加的外部日志处理器列表（例如用于 GUI）。
